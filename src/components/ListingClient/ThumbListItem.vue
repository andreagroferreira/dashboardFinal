...
<b-card @click.prevent="toggleItem($event,data.client.id)" :class="{'d-flex flex-row':true,'active' : selectedItems.includes(data.client.id)}" no-body>
    <router-link :to="`/app/clients/edit-client/${data.client.id}`" class="d-flex">
        <img :src="data.plataform.thumb" class="list-thumbnail responsive border-0" style="width:113px !important; height:85px !important;object-fit: contain !important;" :alt="data.client.name" />
    </router-link>
