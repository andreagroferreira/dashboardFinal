<template>
  <div>
    <b-row>
      <b-colxx xxs="12">
        <piaf-breadcrumb :heading="currentUser.user.name"/>
    ...
    <b-row>
      <b-colxx xl="6" lg="12">
        <div class="icon-cards-row">
          <icon-cards-carousel :pending-leads="parseInt(0)"  :converted-leads="parseInt(0)"  :new-leads="parseInt(0)" :open-tasks="parseInt(0)" ></icon-cards-carousel>
        </div>
        <b-row>
          <b-colxx md="12" class="mb-4">
            <sales-chart-card :labels-chart="labels" :data-chart="dataChart"></sales-chart-card>
          </b-colxx>
        </b-row>
      </b-colxx>
      <b-colxx lg="12" xl="6" class="mb-4">
        <recent-orders :leadsConverted="leadsConverted"></recent-orders>
      </b-colxx>
    </b-row>
  </div>
</template>
...
export default {
  ...
  data()
  {
    return{
      ...
      dataChart:[54, 63, 60, 65, 60, 68, 100],
      leadsConverted: [
        {
          id: 1,
          title: 'Marble Cake',
  ...
  mounted() {
    this.$store.dispatch('ResumeResult')
  }
}
</script>
